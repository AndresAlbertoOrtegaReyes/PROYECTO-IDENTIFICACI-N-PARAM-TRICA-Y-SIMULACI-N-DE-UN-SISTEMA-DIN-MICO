const int IN3 = 7;    // Inpu t3 puente H c onec t ad a a l pin 7 Arduino
const int IN4 = 8;    // Input4 puente H c onec t ad a a l pin 8 Arduino
...
ISR(TIMER2_OVF_vect) {
  ...
  } else if (count >= 14000 && count <= 16000) {
    digitalWrite(IN3, HIGH);
    digitalWrite(IN4, LOW);
  } 
  else if (count >= 18000 && count <= 20000) {
    digitalWrite(IN3, HIGH);
    digitalWrite(IN4, LOW);
  }
  else if (count >= 22000 && count <= 26000) {
    digitalWrite(IN3, LOW);
    digitalWrite(IN4, HIGH);
  }
  else {
